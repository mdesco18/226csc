   ...
   for (int v=0; v<N+2; ++v)
      for (int w=0; w<v; ++w) 
         if (dist(v,w) <= 1000) { // 50*20 = 1000
            ...
            if (fv != fw) union( fv, fw );
         }
   int fS = find( 0 ), fT = find( N+1 );
   return( fS==fT );
}
...
public static void main ( String[] args ) {
  ...
  for (int t=in.nextInt(); t>0; --t) {
     int N = in.nextInt();
     uf = new int[N+2];  sz = new int[N+2];
     x  = new int[N+2];   y = new int[N+2];
     for (int n=0; n<=N+1; ++n) {
        sz[n] = 1;  uf[n] = n;
        x[n] = in.nextInt();  y[n] = in.nextInt();
     }
